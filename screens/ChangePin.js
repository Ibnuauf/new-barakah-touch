...
export default function ChangePin({ navigation }) {
    ...
    const [title, setTitle] = useState('1 - รหัส PIN ปัจจุบัน')
    ...
    const onConfirmPressed = () => {
        ...
        if (alertType === 'authen') {
            ...
            setAlertMessage('รหัส Pin ไม่ตรงกัน')
            setPin([])
            setPinCode(['', '', '', '', '', ''])
            setTitle('1 - รหัส PIN ปัจจุบัน')
            setCurrentPin('')
            setFirstPin('')
            setAlertType('normal')
            navigation.reset({
    ...
    useEffect(() => {
        if (currentPin === '') {
            if (pin.length === 6) {
                setCurrentPin(pin.join(''))
                setPin([])
                setPinCode(['', '', '', '', '', ''])
                setTitle('2 - ตั้งรหัส PIN ใหม่')
            }
        } else {
            if (firstPin === '') {
                if (pin.length === 6) {
                    ...
                    setPin([])
                    setPinCode(['', '', '', '', '', ''])
                    setTitle('3 - ยืนยันรหัส Pin อีกครั้ง')
                }
            } else {
                if (pin.length === 6) {
                    setShowAlert(true)
