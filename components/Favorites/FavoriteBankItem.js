...
import { searchBankIcon, searchBankName } from '../../util'

const FavoriteBankItem = ({ type, item, previousScreen }) => {
    const navigation = useNavigation()

    const { BANK_CODE, FAVORITE_NAME, ACC_TYPE, AMOUNT } = item
    ...
    const onSelectItem = () => {
        if (previousScreen === 'TransactionMenu') {
            navigation.navigate('SendToBank', { favoriteNumber: ACC_TYPE, FavBankCode: BANK_CODE, favAmount: AMOUNT, previousScreen })
        }
    }
    ...
    return (
        <TouchableOpacity style={styles.container} onPress={onSelectItem}>
            <View style={styles.iconBox}>
                {
                    BANK_CODE ? (
                        <Image style={styles.accountIcon} source={searchBankIcon(BANK_CODE)} />
                    ) : (
                        ACC_TYPE.length === 13 ? (
                            <Image style={styles.accountIcon} source={require('../../assets/bank/idcard.png')} />
                        ) : (
                            <Image style={styles.accountIcon} source={require('../../assets/bank/smartphone.png')} />
                        )
                    )
                }
            </View>
            <View style={styles.detailBox}>
                ...
                <Text style={styles.detailTxt} numberOfLines={1}>{BANK_CODE ? searchBankName(BANK_CODE) : 'พร้อมเพย์'}</Text>
                <Text style={styles.detailTxt} numberOfLines={1}>{ACC_TYPE}</Text>
            </View>
...
export default FavoriteBankItem
